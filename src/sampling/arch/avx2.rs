...
// `mask` is masked to 8 bits before indexing the 256-entry table; the
// padded-copy rewrite the lint suggests is this code.
#[allow(clippy::indexing_slicing)]
pub(crate) fn reject(bytes: &[u8], out: &mut RejectBuffer, count: &mut usize) {
    ...
    // run only while `group + 1 < groups`, whose last offset `12 * (groups -
    // 2) + 16` is `len - 8`; the final group is copied into a padded stack
    // buffer first. Each store writes eight `i16` lanes at `out[*count]` with
    // `*count < N` (loop guard), and `N - 1 + 8` is the last slack index of
    ...
    unsafe {
        ...
        while group < groups && *count < parameters::N {
            let data = if group + 1 < groups {
                _mm_loadu_si128(bytes.as_ptr().add(12 * group).cast::<__m128i>())
            } else {
